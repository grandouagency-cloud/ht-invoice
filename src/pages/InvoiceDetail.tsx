...
export default function InvoiceDetail() {
  ...
  // Alias typed as Invoice (not Invoice|undefined) so TypeScript keeps the narrowing
  // inside async closures after await boundaries (TS 5.x strict behaviour)
  const invoice = inv;
  ...
  const handlePDF = async () => {
    ...
    const res = await fetch(`${apiUrl}/api/pdf/generate`, {
      ...
      body: JSON.stringify({ invoice, client, settings }),
    });
  ...
  const handleEmail = async () => {
    ...
    await fetch(`${apiUrl}/api/email/send`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        to: client?.email, invoiceNumber: invoice.number,
        clientName: client?.name, total: invoice.total,
        currency: sym, dueDate: invoice.dueDate,
      }),
    });
    setEmailSent(true);
    setTimeout(() => setEmailSent(false), 3000);
    ...
    <div className="space-y-5 animate-in max-w-4xl">
      ...
      <div className="card p-8 space-y-8">
        ...
        <div className="border-t border-dark-border pt-4 text-center">
          <p className="text-dark-muted text-xs">Generated by <span className="text-brand">ht-invoice</span> · github.com/grandouagency-cloud/ht-invoice</p>
        </div>
      </div>
